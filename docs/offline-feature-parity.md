...
| 4 | Inventory and stock lots | **Partial** | Simple `products.stock_quantity` decrement on sale (atomic, `FOR UPDATE`). **Missing**: `stock_lots`, `stock_movements`, `invoice_item_stock_allocations`, FIFO allocation, manual adjustments, batch-aware purchase entry. Planned migration `0004_stock_lots.sql`. |
