-- Snapshot service transaction metadata on invoice_items via pos_checkout.
--
-- The invoice_items.service_* columns have existed since 0001, but pos_checkout
-- ignored them. This migration replaces pos_checkout (same signature) so that
-- when a cart line is a service it reads the optional service_* fields from
-- the JSON cart entry, validates them, and snapshots them onto the inserted
-- row. All other logic (FIFO allocation, loss prevention, customer ledger,
-- invoice numbering, RLS via security invoker) is preserved verbatim from
-- migration 0009.
--
-- IMPORTANT business rule preserved:
--   For services we keep purchase_price = 0 and stock_quantity unchanged.
--   The service principal (service_transaction_amount) is pass-through and
--   must never appear in profit math. Reports compute service profit as
--   service_commission when present, else line_total (a zero-cost fallback).
...
declare
  ...
  -- Service transaction snapshots (read from cart per service line).
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_cart) loop
    ...
    -- Service transaction validation (only when fields present).
  ...
  for v_item in select * from jsonb_array_elements(p_cart) loop
    ...
    if v_product.type = 'product' then
      ...
      -- Service line: snapshot service_* fields (all optional, default to 0/null).
      declare
        v_item_id uuid := gen_random_uuid();
      begin
        v_svc_principal   := coalesce(nullif(v_item->>'service_transaction_amount','')::numeric, 0);
