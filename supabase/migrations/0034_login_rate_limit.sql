...
-- All DDL uses IF NOT EXISTS / DROP IF EXISTS so re-runs are safe.
...
drop policy if exists "service_role_only — no anon reads"
  on public.login_attempts;
create policy "service_role_only — no anon reads"
  on public.login_attempts for select
  using (false);

drop policy if exists "service_role_only — no anon inserts"
  on public.login_attempts;
create policy "service_role_only — no anon inserts"
  on public.login_attempts for insert
  with check (false);

drop policy if exists "service_role_only — no anon deletes"
  on public.login_attempts;
