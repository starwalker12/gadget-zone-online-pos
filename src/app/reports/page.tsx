...
export default async function ReportsPage({
  ...
  return (
    <AppShell pageTitle="Reports">
      ...
      {/* Potential Profit in Stock (read-only estimate) */}
      <section className="mt-6 rounded-2xl border border-amber-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-2xl font-black text-slate-900">{formatCurrency(potentialProfit.totalInventorySaleValue, currency)}</p>
          </div>
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Inventory Cost Value</p>
            <p className="mt-1 text-2xl font-black text-slate-900">{formatCurrency(potentialProfit.totalInventoryCostValue, currency)}</p>
          </div>
          <div className="rounded-xl border border-amber-100 bg-amber-50/50 p-4">
            <p className="text-xs font-bold text-amber-800 uppercase tracking-wider">Potential Profit</p>
            <p className={`mt-1 text-2xl font-black ${potentialProfit.potentialProfitInStock >= 0 ? "text-amber-950" : "text-red-700"}`}>
              {formatCurrency(potentialProfit.potentialProfitInStock, currency)}
            </p>
            {potentialProfit.marginPercent !== null && (
              <p className="mt-0.5 text-xs font-semibold text-amber-700">
                {potentialProfit.marginPercent}% margin on current stock
              </p>
            )}
          </div>
        </div>
